...
class MoviesListViewController: UIViewController {
    ...
    func setupView() {
        ...
        // add error view
        tableView.addSubviewWithParentConstraints(subView: self.errorLabel,
                                                  edges: UIEdgeInsets(top: .zero, left: 16, bottom: .zero, right: 16),
                                                  useSafeArea: true)
    }
...
extension MoviesListViewController {
    ...
    func bind() {
        ...
        output.$errorMessage
            .receive(on: DispatchQueue.main)
            .map { $0 == nil }
            .assign(to: \.isHidden, on: self.errorLabel)
            .store(in: disposeBag)
